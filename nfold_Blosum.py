...
Antigen = np.load('./NAgdata/V1_B_train_antigen.npy')
HLA = np.load('./NAgdata/V1_B_train_HLA.npy')
label = np.load('./NAgdata/V1_B_train_label.npy')
...
for train_index, test_index in kf.split(Antigen):
    print(test_index)
    # train_data = HLA[train_index], Antigen[train_index], label[train_index]
    # test_data = HLA[test_index], Antigen[test_index], label[test_index]
    CV_5fold_train[current_fold].append(HLA[train_index])
    CV_5fold_train[current_fold].append(Antigen[train_index])
    CV_5fold_train[current_fold].append(label[train_index])
    CV_5fold_test[current_fold].append(HLA[test_index])
    CV_5fold_test[current_fold].append(Antigen[test_index])
    CV_5fold_test[current_fold].append(label[test_index])
    current_fold += 1

print('shape:', CV_5fold_train[2][0].size(),CV_5fold_train[2][1].size())
...
class SelfAttention(nn.Module):
    def __init__(self, hidden_size, num_attention_heads, attention_probs_dropout_prob):
        ...
        self.MHA = MultiHeadAttention(hidden_size, num_attention_heads, attention_probs_dropout_prob)
    ...
    def forward(self, hidden_states):
        ...
        context_layer = self.MHA(mixed_query_layer, mixed_key_layer, mixed_value_layer)
...
class TestBAN(nn.Module):
    def __init__(self, **config):
        ...
        self.bcn = weight_norm(
            BANLayer(v_dim=35, q_dim=17, h_dim=128, h_out=1),
            name='h_mat', dim=None)
        self.mlp_classifier = MLPDecoder(128, 1024, 64, binary=1)
        self.HLA_emb = nn.Linear(20, 128)
        self.Anti_emb = nn.Linear(20, 128)

        self.HLA_CNN = CNN(32,1,34)
        self.Anti_CNN = CNN(32,1,15)
        self.layers = 3
...
if __name__ == "__main__":
    ...
    print("total parameters:%.2fM" % (total / 1e6))
    ...
    for fold in range(folds):
        ...
        train_Dataset = prepare_data(CV_5fold_train[fold][0], CV_5fold_train[fold][1], CV_5fold_train[fold][2])
        train_loader = DataLoader(dataset=train_Dataset, batch_size=128, shuffle=True)


        test_Dataset = prepare_data(CV_5fold_test[fold][0], CV_5fold_test[fold][1], CV_5fold_test[fold][2])
        ...
        for epoch in range(epochs):
            for hla, antigen, Y in tqdm(train_loader):
                hla = torch.Tensor(hla).to(device)
                antigen = torch.Tensor(antigen).to(device)
                Y = torch.Tensor(Y).to(device)
                optimizer.zero_grad()

                score = model(hla, antigen)

                n, loss = binary_cross_entropy(score, Y)

                loss.backward()
                optimizer.step()

                loss_epoch += loss.item()
            loss_epoch = loss_epoch / 64
            print("fold:{}, Epoch:{},loss_epoch:{}".format(fold, epoch, loss_epoch))
        ...
        with torch.no_grad():
            ...
            for testhla, testantigen, testY in tqdm(test_loader):
                ...
                tscore = model(thla, tantigen)
                tn, tloss = binary_cross_entropy(tscore, labels)

                test_loss += tloss.item()
                y_label = y_label + labels.to("cpu").tolist()
                y_pred = y_pred + tn.to("cpu").tolist()
        ...
        if auroc > best_model:
            best_model = auroc
            print("Best model was saved in fold:{}".format(fold))
            torch.save(model.state_dict(), "best_model.pth")

        print('Fold:{}, Accuracy:{}, ROC:{}, PR:{}'.format(fold, accuracy, auroc, auprc))

    mean_AUC = mean_AUC / 5
    mean_AUPR = mean_AUPR / 5
    print("Mean AUC:{}, AUPR:{} in 5-fold cross-validation!!".format(mean_AUC, mean_AUPR))
