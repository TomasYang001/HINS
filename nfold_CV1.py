...
import copy
from CV_index import nfold_cv_index
import random
import math
...
Antigen_B = np.load('./NAgdata/V1_B_train_antigen.npy')
HLA_B = np.load('./NAgdata/V1_B_train_HLA.npy')
HLA_B = torch.FloatTensor(HLA_B)
Antigen_B = torch.FloatTensor(Antigen_B)

Antigen_E = torch.load('./NAgdata/V1_ESM2_Antigen.pt')
HLA_E = torch.load('./NAgdata/V1_ESM2_HLA.pt')
...
CV_nfold_train, CV_nfold_test = nfold_cv_index(Antigen, folds)
train_Antigen, train_HLA, train_label = Antigen[CV_nfold_train[0]], HLA[CV_nfold_train[0]], label[CV_nfold_train[0]]
test_Antigen, test_HLA, test_label = Antigen[CV_nfold_test[0]], HLA[CV_nfold_test[0]], label[CV_nfold_test[0]]
...
class TestBAN(nn.Module):
    def __init__(self, **config):
        ...
        self.HLA_emb = nn.Linear(21, 128)
        self.Anti_emb = nn.Linear(21, 128)
...
if __name__ == "__main__":
    ...
    train_Dataset = prepare_data(train_HLA, train_Antigen, train_label)
    train_loader = DataLoader(dataset=train_Dataset, batch_size=256, shuffle = True)

    test_Dataset = prepare_data(test_HLA,test_Antigen,test_label)
    ...
    for epoch in range(epochs):
        ...
        print("Epoch:{},loss_epoch:{}".format(epoch,loss_epoch))
    ...
    torch.save(model.state_dict(), "DA1.pth")
    ...
    print('Accuracy:{}, ROC:{}, PR:{}'.format(accuracy, auroc, auprc))
