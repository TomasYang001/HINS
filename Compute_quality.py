...
# sys.argv[0] = "NAgdata/NAgs.csv"
NAg = pd.read_csv(sys.argv[1])
NAg = NAg.dropna()
...
class EpitopeDistance(object):
    ...
    def __init__(self, model_file = os.path.join(os.path.dirname(__file__), 'NAgdata', 'epitope_distance_model_parameters.json'), amino_acids = 'ACDEFGHIKLMNPQRSTVWY'):
    ...
    def epitope_dist(self, epiA, epiB):
        ...
        return sum([self.d_i[i]*self.M_ab[self.amino_acid_dict[epiA[i]], self.amino_acid_dict[epiB[i]]] for i in range(len(epiB))])
...
model.load_state_dict(torch.load('DA3.pth', map_location=torch.device('cpu')))
...
for ID in unique_ID:
    print('start')
    patient = NAg.loc[NAg["Patient ID"] == ID, :]
    patient = patient.reset_index(drop=True)

    patient_Antigen, patient_allele = pre_process(patient)
    ...
    count = 0
    for neo in range(len(patient)-1):
        HLA_ = HLA_array[neo]
        HLA_ = HLA_.reshape(1, HLA_.shape[0], HLA_.shape[1])
         
        HLA_ = torch.Tensor(HLA_)
        Antigen_ = WT_peptide_array[neo]
        Antigen_ = Antigen_.reshape(1, Antigen_.shape[0], Antigen_.shape[1])
        Antigen_ = torch.Tensor(Antigen_)
        R = model(HLA_, Antigen_)
        A = np.log(int(patient["WT_affinity"][neo]) / int(patient["MT_affinity"][neo]))

        if len(patient["WT_peptide"][neo]) >= 9:
            try:
                C = epidist.epitope_dist(patient["Antigen"][neo], patient["WT_peptide"][neo])
            except:
                C = 0
        ...
        if quality > 1:
            count += 1
    #     print("quality: ", quality, "Immunogenicity: ", abs(R), "Presentation: ", A, "Reactivity: ", C)
    LgINB = np.log10(count+1)
    clinical_record.append([ID, count, LgINB])
    print("Patient:", ID, "INB", count, "log(INB+1)", LgINB)

Clinical = pd.DataFrame(clinical_record, columns= ["Patient", "INB", "lg"])
Clinical.to_csv("V1_Clincal.csv", index = None)


# print("immunogenic neoantigen burden: ", INB)