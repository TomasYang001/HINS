...
for one_class in HLA_ABC:
    ...
    pseudo_seq_pos=[7,9,24,45,59,62,63,66,67,79,70,73,74,76,77,80,81,84,95,97,99,114,116,118,143,147,150,152,156,158,159,163,167,171,182]
...
def pre_process(dataset):
    ...
    if paired=='F':
        ...
        HLA_antigen=HLA_antigen[HLA_antigen.Antigen.str.len() < 20]
#         print(str(max(HLA_antigen.index)-HLA_antigen.shape[0])+' antigens longer than 15aa are dropped!')
        antigen_list=list(HLA_antigen['Antigen'])
        HLA_list=list(HLA_antigen['HLA'])
        # label_list = dataset['Immunogenicity'].tolist()
    else:
        ...
        dataset=dataset[dataset.Antigen.str.len()<15]
        print(dataset.index)
...
def preprocess(filedir):
    ...
    if paired=='F':
        ...
        HLA_antigen=HLA_antigen.drop(HLA_antigen.iloc[index_list].index)
        HLA_antigen=HLA_antigen[HLA_antigen.Antigen.str.len() < 20]
        # label_list = label_list.drop(label_list.iloc[index_list].index)
#         print(str(max(HLA_antigen.index)-HLA_antigen.shape[0])+' antigens longer than 15aa are dropped!')
        antigen_list=list(HLA_antigen['Antigen'])
        HLA_list=list(HLA_antigen['HLA'])
        label_list = dataset['Immunogenicity'].tolist()
...
aa_dict_one_hot = {'A': 0,'C': 1,'D': 2,'E': 3,'F': 4,'G': 5,'H': 6,'I': 7,'K': 8,'L': 9,
           ...
           'W': 18,'Y': 19,'X': 20}  # 'X' is a padding variabl
# e
aa_dict_one_hot_col = {'A': 0,'C': 1,'D': 2,'E': 3,'F': 4,'G': 5,'H': 6,'I': 7,'K': 8,'L': 9,
           'M': 10,'N': 11,'P': 12,'Q': 13,'R': 14,'S': 15,'T': 16,'V': 17,
           'W': 18,'Y': 19}
...
"""), sep='\s+').loc[list(aa_dict_one_hot.keys()), list(aa_dict_one_hot_col.keys())]
...
BLOSUM62_MATRIX = pd.read_table(StringIO(u"""                                                                                      
   A  R  N  D  C  Q  E  G  H  I  L  K  M  F  P  S  T  W  Y  V  B  J  Z  X  *
...
ENCODING_DATA_FRAMES = {
    ...
    #     for j in range(len(aa_dict_one_hot_col.keys()))
    # ], index=aa_dict_one_hot.keys(), columns=aa_dict_one_hot_col.keys())
}
...
def HLAMap(dataset, encoding_method):
    ...
    for each_HLA in tqdm(dataset):
        if m == 0:
            # print("HLA:", each_HLA)
            HLA_array = hla_encode(each_HLA, encoding_method).reshape(1, 35, 20)
        else:
            HLA_array = np.append(HLA_array, hla_encode(each_HLA, encoding_method).reshape(1, 35, 20), axis=0)
        m = m + 1
    # print('HLAMap done!')
    return HLA_array
...
def antigenMap(dataset, maxlen, encoding_method):
    ...
    m = 0
    for each_antigen in tqdm(dataset):
        if m == 0:
            antigen_array = peptide_encode_HLA(each_antigen, maxlen, encoding_method).reshape(1, maxlen, 20)
        else:
            antigen_array = np.append(antigen_array,
                                      peptide_encode_HLA(each_antigen, maxlen, encoding_method).reshape(1, maxlen, 20),
                                      axis=0)
        m = m + 1
...
if __name__ == '__main__':
    ...
    train_antigen_list, train_HLA_list, train_label_list = preprocess("./NAgdata/V1_IEDB.csv")

    # print("len:",len(train_HLA_list),len(train_label_list))
    train_antigen_array = antigenMap(train_antigen_list, 17, 'BLOSUM62')
    np.save('NAgdata/V1_B_train_antigen.npy', train_antigen_array)

    print('Map Done')

    train_HLA_array = HLAMap(train_HLA_list, 'BLOSUM62')
    np.save('NAgdata/V1_B_train_HLA.npy', train_HLA_array)

    print("HLA Done!")

    train_label_list = np.asarray(train_label_list)
    np.save('NAgdata/V1_B_train_label.npy', train_label_list)
